export const OnDeviceEvalProsAndCons = ({ sdkName, otherSDK }) => (
  <>
    <Info>
    ...
    <div>
      The <code>{sdkName}</code> SDK uses a different paradigm than its precomputed counterpart (<a href={otherSDK.link}>{otherSDK.name}</a>). 
    ...
    - Payload size is strictly larger than what is required for the <a href={otherSDK.link}>{otherSDK.name}</a>
...
export default OnDeviceEvalProsAndCons;
