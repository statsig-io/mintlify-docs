...
- **`dataStore`**: `Optional<DataStore>`  
  ...
  Can also be used to continuously fetch updates in place of the Statsig network. See [Data Stores](#data-store).
...
- **`specsAdapterConfig`**: `Optional<SpecAdapterConfig>`  
  More advanced settings to config SDK to fetch from different sources: for example, [statsig forward proxy](/infrastructure/forward-proxy), your own proxy server, data store. 
  Or using different network protocol, http vs grpc streaming.

- **`observabilityClient`**: `Optional<ObservabilityClient>`  
  Interface for you to integrate observability metrics exposed by SDK, including, config propagation delay, initialization time spent. See [details](#observability-client)

- **`persistentStorage`**: `Optional<PersistentStorage>`  
  Interface for you to user persistent storage within sdk. To view more [details](#persistent-storage)
