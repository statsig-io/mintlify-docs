...
- If your application relies on these features being ready **immediately after** `initialize()`, you can opt in by setting:
  - <code>{waitForUserAgentInit }</code>
  - <code>{waitForCountryLookupInit}</code>
