...
The server SDK supports generating the `initializeValues` needed to initialize a Statsig Client SDK, letting you pair your client-side initialize() calls with other requests to your server, effectively enabling zero-latency client SDK startup.

{/* Client initialization code snippet should be included here */}
...
fields on the user object when calling the initialization method.
