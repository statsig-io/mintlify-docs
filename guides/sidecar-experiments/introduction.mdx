...
sidebarTitle: Sidecar Introduction
...
Customers that are very performance-minded typically use our [JS-SDK](/sdks/client/javascript) for testing on the web. When using the JS/React SDKs with [Client Bootstrapping](/sdks/client/javascript#bootstrap-initialization), the latency introduced is very minimal since there are no network requests required for initialization.
