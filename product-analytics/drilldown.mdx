...
When a gate is checked for a user, an exposure is created for the rule who's conditions they've met. If the user is exposued to multiple different rules at different times, the first exposure from each rule is kept. We recommend grouping by rule to see each rule's exposures separately.
