...
:::note 
Note: the `@statsig/serverless-client/cloudflare` package only supports JavaScript. 
:::

First up, you'll need to install the Statsig serverless sdk.
...
- `options : Statsig Options Object` If you plan to log events, set `{loggingEnabled: 'always'}` like in the example provided. See here, for more [options](https://docs.statsig.com/client/javascript-sdk#statsig-options).
...
```
This is an event log in code. For more information on event logging, see [here](https://docs.statsig.com/client/jsOnDeviceEvaluationSDK#logging-an-event).

The `logEvent` method takes two parameters:
- `Event name : string` This is the name of the event you are logging
- `StatsigUser` The Statsig user object for whom the event is being logged.

Refer to the documentation above for more optional parameters for more thorough event logging

:::note 
Note: `loggingEnabled: 'always'` must be set in the options object when you create an instance of the client to successfully log events
:::
...
To optimize for edge use cases, we do not provide an api to recognize updates to your config specs. However, when a change is made to your project definition on the Statsig console, the changes will be propagates to the KV store and will be reflected the next time you initialize the Cloudlfare client.
```
