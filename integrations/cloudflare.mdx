...
- `params : ParamsObject` :
    ...
    |-----------|----------|------|-------------|
    | `kvKey` | No | `string` | The key of your KV pair |
    | `envStatsigKey` | No | `string` | The **env var** storing your Statsig client key |
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      const initResult = await client.initialize(env.STATSIG_KV, env.kv_key);
